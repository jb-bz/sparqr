...
### Planned for v0.4.1 ("Make it a methodology")
- `sparc story` command — manage story-point estimates per repo (3 pts)
- `sparc config validate` warns on 13-pt stories (2 pts)
- `sparc retro` command — auto-generate retrospective file (3 pts)
...
features; v0.4.1 makes the underlying methodology reproducible —
story points, retrospectives, velocity tracking, and post-commit
nudges — so the practice survives across sessions and other humans
can adopt it. **All 5 stories shipped (13 pts).**

### Added
- **`sparc story` (story 4a, 3 pts).** Per-repo story-point
  ledger at `.sparc/stories.yaml`. Subcommands: `add`, `list`,
  `show`, `update`, `split`, `rm`. Fibonacci 1/2/3/5/8/13. 13-pt
  stories emit a warning + split reminder (warnings don't block).
  Split stories become top-level entries linked via `parent_story`.
- **`sparc config validate` warns on 13-pt stories (story 4b, 2 pts).**
  After schema validation, reads `.sparc/stories.yaml` and emits
  a warning for each 13-pt story in `planned` / `in-progress`
  status. Warn-don't-fail: exits 0 (the warnings don't block CI).
- **`sparc retro` (story 4c, 3 pts).** Auto-generates a
  retrospective file at `docs/retrospectives/<release>.md`
  (or `<release>-WIP.md` to avoid clobbering finalized retros).
  The "What surprised us" section is generated by analyzing
  commit messages between the previous and current release tag —
  detects bug-fix patterns, docs work, perf changes, dependency
  updates, test coverage gaps. Outputs real prose, not boilerplate.
  Falls back to cwd for CHANGELOG/ROADMAP, not the package root.
- **`sparc velocity` (story 4d, 2 pts).** Reads retros +
  `.sparc/stories.yaml`, prints a table:
  `RELEASE  EST  ACTUAL  RATIO  DONE/TOTAL  DEFERRED  NOTES`.
  Flags under/on-target/over delivery. JSON and CSV output
  formats supported.
- **Post-commit hook (story 4e, 2 pts).** Optional hook installed
  by `sparc init` (y/N prompt). Detects when a release tag was
  just added or the commit message references a version, prints
  a non-blocking reminder to run `sparc retro <tag>`.

### Changed
- **`sparc init` now prints a Tip line** reminding the user to
  track story points with `sparc story add`. This makes the
  methodology discoverable instead of hidden in the docs.

### Stats
- 5 stories, all shipped on schedule.
- 416 unit tests + 11 integration assertions, all passing.
  (Was 339 before v0.4.1; +77 new test assertions.)
- Velocity: 1.00 (12 estimated, 12 actual).

[0.4.1]: https://github.com/jb-bz/sparqr/releases/tag/v0.4.1
